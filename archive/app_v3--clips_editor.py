...
from PyQt5.QtWidgets import (
    ...
    QLineEdit, QStackedLayout, QScrollArea, QCheckBox, QDialog
)
    ...
    def paintEvent(self, event):
        ...
        for clip in self.clips:
            ...
            if clip.label is not None:
                ...
                # if clip.reasons:
                #     text = f"{text} ({', '.join(r[0] for r in clip.reasons)})"  # Show first letter of each reason
...
class LabelDetailsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        container = QWidget()
        container.setStyleSheet("background-color: white;")  # Ensure container is also white
        self.checkbox_layout = QVBoxLayout(container)
        self.checkbox_layout.setSpacing(2)  # Reduce space between checkboxes
    ...
    def set_label_type(self, label_type: Literal['Accept', 'Reject']):
        """Update checkboxes based on label type"""
        ...
        # Add new checkboxes based on label type
    ...
    def accept(self):
        ...
        super().accept()  # Call parent class accept() method
        
    def reject(self):
        self.selected_reasons = []
        super().reject()  # Call parent class reject() method
...
class VideoPlayer(QMainWindow):
    def __init__(self):
        ...
        # Add clips widget
        self.clips_widget = ClipsWidget(self)
    ...
    def play_video_at_index(self, index):
        ...
        if 0 <= index < len(self.video_list):
            ...
            logger.info(f"[Player] Playing video at index {index}, file:{self.video_list[index]}, SHA-256:{self.video_file_hash}")
            self.current_video_index = index
            
            # Open video first to get total frames
            self.open_video(self.video_list[index])
            
            # Clear clips widget state and create initial full clip
